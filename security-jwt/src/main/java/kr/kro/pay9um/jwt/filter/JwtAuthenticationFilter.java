package kr.kro.pay9um.jwt.filter;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (StringUtils.hasText(token) && jwtTokenValidator.validateToken(token, TokenType.ACCESS)) {
            try {
                String uid = jwtTokenProvider.extractUid(token);
                UserDetails userDetails = customUserDetailsService.loadUserByUid(uid);
